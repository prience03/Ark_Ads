...
final public class Run {
                    ...
                    Thread thread = new Thread("ThreadRunHandler") {
                        ...
                        public void run() {
                            ...
                            synchronized (Run.class) {
                                ...
                                // And the async dif to sync method
                                backgroundPoster = new HandlerPoster(Looper.myLooper(), 3 * 1000,
                                        true);
                                // notify run next
                                try {
                                    Run.class.notifyAll();
    ...
    /**
     * Asynchronously The current thread asynchronous run relative to the sub thread, not blocking
     * the current thread
     *
    ...
    /**
     * Asynchronously The current thread asynchronous run relative to the main thread, not blocking
     * the current thread
     *
    ...
    /**
     * Synchronously The current thread relative thread synchronization operation, blocking the
     * current thread, thread for the main thread to complete
     *
     * @param action Action Interface
     */
    ...
    /**
     * Synchronously The current thread relative thread synchronization operation, blocking the
     * current thread, thread for the main thread to complete But the current thread just wait for
     * the waitTime long.
     *
     * @param action          Action Interface
     ...
     * @param cancelOnTimeOut on timeout the current thread cancel the runnable task
     */
    public static void onUiSync(Action action, long waitMillis, int waitNanos,
     ...
     * <p/>
     * The current thread relative thread synchronization operation, blocking the current thread,
     ...
     * <p/>
     * The current thread relative thread synchronization operation, blocking the current thread,
     * thread for the main thread to complete But the current thread just wait for the waitTime
     * long.
     *
     * @param func            Func Interface
     * @param waitMillis      wait for the main thread run milliseconds Time
     ...
     * @return {@link T}
     */
    public static <T> T onUiSync(Func<T> func, long waitMillis, int waitNanos,
            boolean cancelOnTimeOut) {
        if (Looper.myLooper() == Looper.getMainLooper()) {
            return func.call();
        }
